...
Tabela \ref{tab:cirkulantni_4} prikazuje rezultate za $f(k) = 2(k-1)^2 - 3$, medtem ko tabela \ref{tab:cirkulantni_5} prikazuje rezultate za $f(k) = 2(k-1)^2 - 4$. 
...
\end{itemize}
Želimo ugotoviti ali za $n \geq 7$ velja $dim(C_n(1, \dots, n - 4)) = \lceil \frac{2n}{5} \rceil$.\\
...
V nalogi sva na mesto $k$ uporabljali $m = n - k$, saj je $k$ odvisen od $n$.
Rezultate sva shranili v excel tabeli.

Ustvarili sva seznam s pomočjo for zanke, ki za vsak $n$ izračuna $\lceil \frac{2n}{5} \rceil$. Dobljene vrednosti, pa sva na to primerjali
...
Najini rezultati so pokazali, da velja enakost med vsemi izračunanimi dimenzijami in $\lceil \frac{2n}{5} \rceil$.\\

\noindent \textbf{Sklep:}
Za vsak $n \in [7, 80]$ velja $dim(C_n(1, \dots, n - 4)) = \lceil \frac{2n}{5} \rceil$.


\subsection*{Naloga 2.1}
V tem delu projekta sva eksperimentalno preverili tudi obnašanje metrične dimenzije če uzameva $dim(C_n(1, \dots, n - k))$, kjer 
sva vzeli $k \in \{5, 6, 7, 8, 9, 10\}$. Označevali sva $m = k$, saj sva v funkcijah uporabljali $n - m$.\\
...
\begin{itemize}
    \item Za $m = 5$: Iz rezultatov sva opazili, da metrično dimenzijo lahko opiševa s funkcijo
    \[\dim(C_n(1,2,\dots,n-5)) =
        \begin{cases}
            \dfrac{n}{3}, & \text{če } 3 \mid n, \\[10pt]
            \left\lceil \dfrac{n}{3} \right\rceil + 1, & \text{sicer}.
        \end{cases}\]
    Definirali sva to funkcijo in zračunali njene vrednosti za $n \in [7, \ldots ,80]$. 
    {\scriptsize
    \begin{verbatim}
        def dimenzija_m5(n):
            if n % 3 == 0:
    ...
    Nato, pa sva rezultate te funkcije
    primerjali z izračunanimi metričnimi dimenzijami in ugotovili, da se ujemajo za vse $n \in [9, \ldots, 80]$.
    {\scriptsize
    \begin{verbatim}
    ...
    ki bi jih opisala. Tu nisva imeli take sreče, saj nisve uspele najti funkcije, ki bi natančno opisala vse izračunane dimenzije.
    Kljub temu, pa nama je uspelo najti približek: 
    \[
    ...
    \end{cases}
    \]
    Ta približek je veljal za vse $n \in [11, \ldots, 80]$. Njegovo veljavnost sva preverili s podobno kodo kot prej.
\end{itemize}
